    ...
    >
      <div className="landing-content">
        ...
        <form className="landing-search" onSubmit={handleSubmit} role="search" aria-label="Buscar consulta legal">
          <motion.div className="landing-search-container" layoutId="chat-input-container" style={{ position: 'relative' }}>
            {pendingAttachment && (
            ...
            <div className="chat-input-leading-actions">
              ...
              <button type="button" className="chat-attach-btn" onClick={() => fileInputRef.current?.click()} aria-label="Adjuntar documento" title="Adjuntar PDF o Imagen">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48" />
                </svg>
              </button>
            </div>
            <label htmlFor="landing-search-input" className="visually-hidden">Tu consulta legal</label>
            <input id="landing-search-input" className="landing-search-input" type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Haz tu consulta legal..." autoComplete="off" />
            <div className="landing-search-actions">
              <button id="landing-submit-btn" className="landing-search-btn" type="submit" disabled={!input.trim() && !pendingAttachment} aria-label="Enviar consulta">
        ...
        <section className="landing-prompts" aria-label="Consultas de ejemplo">
          ...
          <div className="landing-prompts-grid" role="list">
            {visiblePrompts.map((prompt, i) => (
              <button key={i} className="prompt-card" onClick={() => handlePromptClick(prompt.question)} role="listitem" aria-label={`${prompt.category}: ${prompt.question}`}>
                <span className="prompt-card-icon" aria-hidden="true">{CATEGORY_ICONS[prompt.category] || <span style={{fontSize: '20px'}}>?</span>}</span>
                <div className="prompt-card-content">
                  <span className="prompt-card-category">{prompt.category}</span>
                  <p className="prompt-card-question">{prompt.question}</p>
                </div>
                <svg className="prompt-card-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
              </button>
            ))}
            <div className={`prompts-expandable ${showAllCards ? 'expanded' : ''}`}>
              {hiddenPrompts.map((prompt, i) => (
                <button key={i + 3} className="prompt-card" onClick={() => handlePromptClick(prompt.question)} role="listitem" aria-label={`${prompt.category}: ${prompt.question}`}>
                  <span className="prompt-card-icon" aria-hidden="true">{CATEGORY_ICONS[prompt.category] || <span style={{fontSize: '20px'}}>?</span>}</span>
                  <div className="prompt-card-content">
                    <span className="prompt-card-category">{prompt.category}</span>
                    <p className="prompt-card-question">{prompt.question}</p>
          ...
          <button className="prompts-toggle" onClick={() => setShowAllCards(!showAllCards)} aria-expanded={showAllCards}>
            {showAllCards ? 'Ver menos' : 'Ver más consultas'}
            <svg className={`prompts-toggle-icon ${showAllCards ? 'rotated' : ''}`} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><polyline points="6 9 12 15 18 9" /></svg>
          </button>
        </section>
      </div>
    </motion.div>
